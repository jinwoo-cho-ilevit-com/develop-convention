...
There is no separate planning-depth setting. How hard a plan is challenged follows the done level: `auto` skips it, `reviewed` gets one adversarial pass, `proven` gets the full lane set. The same level sets the depth at the two points [20-review-gate.md](20-review-gate.md) §2 fixes by time: `reviewed` runs the point's lane once, `proven` runs it twice with the second pass on a different model family. A second dial would only be another thing to under-report.
