...
- Add a fresh-reader lane only when the deliverable is an explainer document (→ [24-explainer-docs.md](24-explainer-docs.md)): its input is the document alone plus the declared target reader — no code, no author context — and it answers by re-explaining each mechanism in its own words. A mechanism it cannot re-explain, or a term used without a gloss, is a finding.
...
| Fresh reader *(conditional)* | the explainer document alone + its declared target reader ([24-explainer-docs.md](24-explainer-docs.md)) | mechanisms that cannot be re-explained from the document, unglossed terms, concepts without examples, structure described without a visual |
