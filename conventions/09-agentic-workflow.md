...
Standard pattern: **plan → define shared contracts → split along non-overlapping ownership boundaries → (for independent execution) worktree isolation → per-task checks → one integration verification pass after merge**
...
- **Integration**: each branch requires passing lint + tests as a merge precondition. Run one integration run — the assembled project's sample run — after merge.
- **Cleanup closes the lane**: after the integration pass, remove each merged lane's worktree and delete its branch. `git worktree remove` refusing a dirty tree and `git branch -d` refusing an unmerged branch are the safety checks working — investigate, do not escalate to `--force`/`-D`. Halted lanes keep their worktrees: fix rounds continue in them.
