...
- A merged lane is a closed lane: once the integration runs are green and the merged-whole review round has closed ([20-review-gate.md](20-review-gate.md) §1), remove its worktree and delete its branch — `git worktree remove` without `--force` and `git branch -d`, never `-D`, because both refusals are safety signals, not obstacles. A halted lane keeps both; its worktree is where the fix round resumes. A branch that survives its merge reads as unfinished work to the next session.
...
Where coordination and isolation sit in the loop as a whole, and what runs before and after them, is [21-development-loop.md](21-development-loop.md) §1.
...
- **Integration**: the merge precondition for each branch is its lane's completion criteria and the CI-enforced checks such as lint passing (→ [18-work-contract.md](18-work-contract.md), [03-environment.md](03-environment.md)), with its review closed on no blocker. Run the integration runs — one to three per project, each the assembled project's sample run — on the merged head after the last merge.
- **Cleanup closes the lane**: once the integration runs are green and the merged-whole review round has closed ([20-review-gate.md](20-review-gate.md) §1), remove each merged lane's worktree and delete its branch. `git worktree remove` refusing a dirty tree and `git branch -d` refusing an unmerged branch are the safety checks working — investigate, do not escalate to `--force`/`-D`. Halted lanes keep their worktrees: fix rounds continue in them.
- **More agents is not automatically faster**: without isolation, scoping, and verification, merge/review cost offsets the gains from parallelism. No lane count is known to be the right one and this document names no threshold. Record the end-to-end wall-clock time, from dispatch to a merged and verified change, and leave it there: the same work cannot be re-run afterwards with fewer lanes, so there is no baseline to turn that number into a ratio against, and a speedup claimed without a comparison run is an estimate (→ the METR case in [00-principles.md](00-principles.md)).
