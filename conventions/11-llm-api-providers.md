...
Sources: [Requesty — empirical test of structured output compatibility](https://requesty.ai/blog/structured-outputs-across-llm-providers-the-compatibility-mess)

### 2. Capability Summary (as of 2026-07 — this changes fast, re-check official docs before adopting)

| | Structured output | Reasoning control | Seed | Caveats |
|---|---|---|---|---|
| OpenAI | `json_schema` strict (Responses: `text.format` / Chat: `response_format`) | `reasoning.effort` (levels vary by model) | Yes (best-effort) + `system_fingerprint` | Responses/Chat parameter names differ. Rejects temperature/top_p during reasoning |
| Anthropic | `output_config.format` json_schema GA (constrained decoding) | adaptive thinking + `effort` (`budget_tokens` deprecated) | **None** | `max_tokens` required. Latest models return 400 for non-default temperature/top_p/top_k |
| Gemini | `responseSchema` (+`response_json_schema`) | `thinking_level` (older generations use `thinking_budget`) | Yes (best-effort) | Duplicating the schema in the prompt degrades quality — responseSchema only |
| DeepSeek | **`json_object` only, no schema enforcement** | thinking toggle (model axis) | — | The word "json" is required in the prompt + examples recommended. Docs explicitly note empty content may be returned |
| OpenRouter | `response_format` pass-through — **only some providers support it** | pass-through, varies by provider | varies by provider | Without `require_parameters: true`, requests get silently routed to a provider that doesn't support the schema |
...
- Reasoning models reject temperature/top_p when `reasoning_effort != none`. Effort levels and their defaults differ by model generation (some models default to `none`), so specify it explicitly in config.
- Record `seed` + the response `system_fingerprint` — a fingerprint change signals a backend change (reproducibility invalidated).
...
- No seed, `max_tokens` required, latest models default to thinking and are controlled via `effort` — manual `budget_tokens` is deprecated.
- 529 (overloaded) is a global capacity signal and a failover target (→ [10](10-llm-api-inference.md) §3).

**Gemini**
- Put the schema only in `responseSchema` — official docs state that duplicating it in the prompt degrades quality.
- Combining thinking with structured output has reported fragility at the wrapper layer, so verify with integration tests at the raw SDK level before use.
- Output tokens may include thinking tokens — reflect this in the token aggregation mapping.
...
- Thinking mode constraints: temperature/top_p etc. are silently ignored (no error), while `logprobs` returns a 400 error — record the distinction between "ignored" and "fails" in the capability table.
- Model name aliases (`deepseek-chat`/`deepseek-reasoner`) have a deprecation schedule, so don't hardcode them — manage via a config axis (model + thinking toggle).
- Rate limiting isn't RPM-based; it's a concurrency cap + dynamic throttling, and there's peak-time surge pricing (check the official pricing page for current figures).

**OpenRouter**
- **HTTP 200 + error body**: upstream failures arrive as an error in the body (or SSE event) of a 200 response. Checking only the status code would treat a failure as success, so checking the body/stream for errors is mandatory.
- Without setting `provider: { require_parameters: true }`, requests get silently routed to a provider that doesn't support parameters like structured output, and the constraint disappears.
- You can leverage the dual structure of within-model provider failover + `models`-array-based model fallback. If it fails after exhausting failover, you aren't charged (zero-completion insurance).
...
Sources: same as §2 above + [DeepSeek thinking mode](https://api-docs.deepseek.com/guides/thinking_mode), [DeepSeek pricing](https://api-docs.deepseek.com/quick_start/pricing), [OpenRouter BYOK](https://openrouter.ai/docs/guides/overview/auth/byok), [OpenRouter reliability/failover](https://openrouter.ai/blog/insights/reliability-failover)
...
5. **Validation + bounded retry**: on Pydantic validation failure, feed the error message back and retry (the instructor approach). Cap retries at 2-3 (each retry incurs the full call cost), and **track the retry rate as a metric** — a rising retry rate is an early signal of a prompt/model problem. In an empirical single-model benchmark (n=1000), 3 validation-retry rounds reported ≈99.6% and a hybrid with repair combined reported ≈99.9% validity (this varies by model/task, so measuring it yourself is recommended).
