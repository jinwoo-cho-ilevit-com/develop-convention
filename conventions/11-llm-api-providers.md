...
| Anthropic | `output_config.format` json_schema GA (constrained decoding) | adaptive thinking + `effort`; `thinking.type: "enabled"` + `budget_tokens` deprecated on 4.6, 400 on 4.7+; effort values `low`/`medium`/`high`/`xhigh`/`max`, default `high` (same as omitting), `xhigh` support is model-specific (as of: 2026-09) | **None** | `max_tokens` is not required and has a minimum of `0` (checked 2026-09-19). Latest models return 400 for non-default temperature/top_p/top_k |
| Gemini | `response_format` (`type`/`mime_type: "application/json"`/`schema`) | `thinking_level` | No `seed` field documented (→ [10](10-llm-api-inference.md) §7) | Schema goes in `response_format.schema`; use `description` fields to steer |
...
Sources: [OpenAI structured outputs](https://developers.openai.com/api/docs/guides/structured-outputs), [OpenAI reasoning](https://developers.openai.com/api/docs/guides/reasoning), [Anthropic structured outputs](https://platform.claude.com/docs/en/build-with-claude/structured-outputs), [Anthropic effort](https://platform.claude.com/docs/en/build-with-claude/effort), [Anthropic thinking](https://platform.claude.com/docs/en/build-with-claude/thinking), [Anthropic extended thinking (deprecation notice)](https://platform.claude.com/docs/en/build-with-claude/extended-thinking), [Anthropic Messages API reference (`max_tokens` not required, minimum 0, checked 2026-09-19)](https://platform.claude.com/docs/en/api/messages), [Gemini structured output](https://ai.google.dev/gemini-api/docs/structured-output), [Gemini thinking](https://ai.google.dev/gemini-api/docs/thinking), [DeepSeek JSON mode](https://api-docs.deepseek.com/guides/json_mode), [DeepSeek pricing (model names, checked 2026-09-19)](https://api-docs.deepseek.com/quick_start/pricing), [DeepSeek thinking mode (checked 2026-09-19)](https://api-docs.deepseek.com/guides/thinking_mode), [OpenRouter structured outputs](https://openrouter.ai/docs/guides/features/structured-outputs)
...
- No seed. Latest models default to thinking and are controlled via `output_config.effort`; manual extended thinking (`thinking.type: "enabled"` with `budget_tokens`) is deprecated on the 4.6 models (requests still succeed) and rejected with a 400 error on 4.7 and later. Migration: drop `budget_tokens`, set `thinking: {type: "adaptive"}`, control depth with `effort`. `max_tokens` is not required and has a minimum of `0` — the reference notes `0` can be used to populate the prompt cache without generating a response (checked 2026-09-19).
- Changing the top-level `output_config.effort` value between requests invalidates the prompt cache (→ [10](10-llm-api-inference.md) §5). Claude Fable 5.1, Claude Mythos 5.1, and Claude Opus 5 can instead change effort per message, keeping the cache: beta header `mid-conversation-output-config-2026-07-01`, add a `role: "system"` message with empty `content` carrying the new `output_config.effort`. Other models, such as Claude Fable 5, return a 400 error for this per-message form. (as of: 2026-09)
- On Claude Opus 5, `thinking: {"type": "disabled"}` combined with `xhigh` or `max` effort returns a 400 error. (as of: 2026-09)
- 529 (overloaded) is a global capacity signal and a failover target (→ [10](10-llm-api-inference.md) §3).

**Gemini**
- No `seed` field is documented on `GenerationConfig` — treat Gemini as offering no comparable determinism knob (→ [10](10-llm-api-inference.md) §7).
...
- Thinking mode constraints: "Thinking mode does not support the `temperature`, `presence_penalty`, or `frequency_penalty` parameters" — setting one has no effect rather than raising an error. `top_p` is not in that list: "`top_p` takes effect in thinking mode, but with a lower bound of `0.95`: values below `0.95` are raised to `0.95`" (checked 2026-09-19). `logprobs` returning a 400 error is **(unverified — needs research: not stated on the current thinking-mode guide)**. Record the "ignored" / "clamped" / "fails" distinction per parameter in the capability table, not a single bucket.
- Model names churn — including inside this very paragraph: `deepseek-v4-pro` is the current pro-tier name. The current flash-tier name is reported as `deepseek-flash`, but that exact string is itself **(unverified — needs research: not independently re-fetched here, and dropping the `v4` tag while `deepseek-v4-pro` keeps it is asymmetric enough to also read as a transcription slip rather than confirmed product naming)** — re-check the pricing page directly rather than trusting the string as printed. What is documented with more confidence is that `deepseek-v4-flash` is at most a legacy name still accepted, not a name to add to new code, and that `deepseek-chat`/`deepseek-reasoner` no longer appear at all (no deprecation schedule is published — they are simply gone) (checked 2026-09-19). Thinking is now a request parameter (`thinking: {"type": "enabled"/"disabled"}`, enabled by default at `effort: high`), not a separate model. Don't hardcode model names — manage via a config axis (model + thinking toggle), and re-check the registry before trusting any name printed in a convention doc, including this one.
- Rate limiting isn't RPM-based; it's a per-model concurrency cap that returns HTTP 429 when exceeded — last documented as 2500 for the flash-tier model (under the pre-churn name `deepseek-v4-flash`; whether the rate-limit page has since moved to `deepseek-flash` is **(unverified — needs research, given the name churn in the bullet above)**) and 500 for `deepseek-v4-pro`. "Dynamic throttling" is **(unverified — needs research: the rate-limit page describes fixed caps + 429)**. Peak/off-peak pricing applies: peak hours (09:00–12:00 and 14:00–18:00 Beijing time, UTC+8) are 2x regular prices on all billing items (check the official pricing page for current figures).
