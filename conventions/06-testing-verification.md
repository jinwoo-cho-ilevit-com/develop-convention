...
- The boundary sample holds every member of the value set its contract file names, as rows. It is the consumer lane's sample-run input, and that run checks only what the sample carries: a value no row contains is a value whose mishandling nothing catches. The producer side is checked by review — its lane's reviewers and the merged-whole review compare the code against the contract file.
...
| Contract (a file, not a test) | a boundary where parallel lanes meet | one per lane boundary | nothing by itself — it is what both lanes build against, checked by each lane's review, the consumer's sample run and, after the merge, the integration run and merged-whole review |
