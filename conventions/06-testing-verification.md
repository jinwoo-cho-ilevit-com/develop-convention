...
The check runs through `check-jsonschema`, pinned to one version so a lane, the freeze check and CI measure with the same tool: `uvx check-jsonschema@0.38.0 --schemafile <schema> <instance>`. It reads JSON, YAML and TOML instances — its `--force-filetype` accepts only `json`, `toml` and `yaml` — so a stage whose dump is JSON Lines or Parquet has no schema, and that boundary stays with review. It exits 1 when an instance fails, and also when the instance file is missing, which is what makes a stage that wrote no dump fail its criterion rather than pass it.
