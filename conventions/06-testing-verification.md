...
The criteria table's `red` column records which of these established a row. [19-evidence.md](19-evidence.md) §1 declares that column and the words it accepts; what each of them means is this section: `observed` is a check seen failing at the base commit, `sabotage` is one confirmed by breaking the behaviour it pins (below), and `guard` is a standing invariant exempt from red. None of the three is a row of the outcome table above, which classifies one check's result rather than how a criterion's red was established.
