...
- **Single reading column.** An explainer is read in order, each section standing on the one before it; a two-column flow splits that path into two tracks the reader must reconcile, and forces exception CSS wherever a section outgrows its column. A readable measure for Korean body text is about 45 to 50 characters per line: Butterick's Practical Typography puts a comfortable line length at 45-90 characters, or 2-3 alphabets ([Butterick, Line Length](https://practicaltypography.com/line-length.html), checked 2026-09-19). A full-width Hangul character occupies roughly twice the horizontal space of an average Latin character, so a line of 45-50 of them spans up to and slightly past the top of that range.
- **Role placement on figures follows the ARIA cascade** (ARIA: the markup spec that gives assistive technology a role for each element). A `role="img"` on `<figure>` would carry down: browsers apply role `presentation` to every descendant of an `img`-role element ([MDN, ARIA img role](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/img_role): "browsers automatically apply role `presentation` to all descendant elements of any `img` element", checked 2026-09-19). A static figure avoids that by putting the role on the inner `<svg>` instead, per the Core Rules.
