...
- Ship an HTML explainer as one self-contained file: no external network dependencies, its character encoding declared inside the file, fonts taken from the system stack rather than embedded as `@font-face` or base64 data, diagrams inline as SVG, and all text selectable and greppable — never rendered into an image.
...
- Give every figure in an HTML explainer the same accessibility contract. A static figure puts no role on `<figure>` and names the drawing on the `<svg role="img" aria-label="short name">` inside — a `role="img"` on the `<figure>` makes the img role's children presentational ([MDN, ARIA img role](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Reference/Roles/img_role): "browsers automatically apply role `presentation` to all descendant elements of any `img` element", checked 2026-09-19), so everything the figure holds drops out of assistive technology while the page still looks correct on screen. A figure with focusable marks becomes `<figure role="group" aria-label="short name">`, and every mark carries an accessible name and a focus indicator the page draws itself rather than leaning on a browser default it does not control. Controls are real elements — `button` with `aria-pressed`, `role=tablist/tab/tabpanel` moved by arrow keys and wired by ids that stay unique, a native range input whose value is always shown as text — a tooltip opens on both hover and focus and closes on Escape, meaning is never carried by color alone, and any motion respects `prefers-reduced-motion`.
...
|---|---|---|---|
| Structure | flow, sequence, hierarchy, topology, comparison of parts | Mermaid | inline SVG |
...
In HTML the decision starts in the "Showing" column and ends in a drawing of that mechanism. It never starts from a form the author liked elsewhere: a chart chosen first and filled afterwards is decoration that happens to hold data.
...
An HTML explainer is the same genre in a rendering that must survive being handed around — opened from mail and chat, offline:

- **Self-contained single file.** No external network dependencies — CDN scripts, remote fonts, hotlinked images. Anything remote is a part of the document that silently disappears. A font embedded as base64 is the opposite failure: not remote, but dead weight in every copy, so text uses the system stack.
- **Encoding declared in the file.** Opened over `file://` there is no transport header to carry it, and a page that leaves the encoding to the viewer's locale is a page whose Korean text is mojibake on someone else's machine.
- **Diagrams inline.** SVG in the file itself, chosen by the same triggers as §3.
...
### 6. Definition first, analogy throughout
