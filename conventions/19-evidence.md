...
The toolkit a project copies out writes the four files above, plus the per-phase records described below. Its `manifest.json` carries `created_at` — written once into the state directory by whichever phase runs first and read back from there, so it is the time the evidence began rather than the time it was last rewritten, which is what §4 derives lead time from — the run's commit and whether the tree was clean, the contract's `base` and `done_level`, the interpreter and platform, the names of the environment variables whose values it masks wherever they appear — those whose names match and whose values clear the length floor below, not every name that matched, which would tell a reviewer a value was hidden while the log still carried it — the human verdicts, `verify_runs[].at`, and the bypass record when the contract carries one. One thing §4 and the Core Rules above require of a `manifest.json` is missing, and a reader diffing the two sections should find it named here rather than infer it: there is no `review_rounds`, because the count belongs to a review subcommand this runner does not have.

`verify_runs` has to accumulate across runs and a per-criterion record holds only the latest, so the verify phase appends to a run log inside the state directory and the manifest is rendered from that — the manifest stays derived from the state directory rather than merged into, which is what stops one phase from losing another's entry. A bypass reaches the manifest the same way, from the contract: `done_level: bypassed` is accepted only with a `bypass` mapping carrying a reason and refused without one, so the level and its record cannot come apart, and the state 18 calls the blocker — a bypass that leaves no trace — is unrepresentable rather than merely discouraged. Five subcommands — `lint`, `red`, `verify`, `human`, `status` — and four exit codes: `0` the phase's answer is yes, `1` the answer is no, `2` the runner could not answer at all, `3` the runner itself broke and printed a traceback. A broken contract is `2` rather than `1` so a caller can tell "your contract is unusable" from "your work is not done"; the two were the same number in the version this replaces, and that is how a gate opened on an unreadable contract. `3` exists for the same reason one step further out: an uncaught exception exits `1` by default, which would read as a criterion having failed.

Each of `red`, `verify` and `human` writes its own record at `artifacts/<feature>/state/<criterion>.<phase>.json`, and the path is derived from the record rather than passed in. A fourth phase name appears in the execution records but writes no state: `red-collect`, the probe that asks pytest which files a criterion selects. It is recorded because the runner executes it, and because a criterion ruled out at collection has no other row — its `NO-BASELINE` would otherwise rest on a command the evidence never mentions. `lint` and `status` write no record at all, and both report any other feature under `artifacts/` still holding a `PENDING-HUMAN` record. The runner reads one contract at a time, so a contract awaiting verdicts can be replaced by the next one with nothing noticing; it cannot refuse, because collecting every verdict at the end leaves several contracts pending on purpose, so it says what it sees. No phase can overwrite another's result because no phase names another's file. `REPORT.md` and `manifest.json` are then re-rendered from that directory, so neither accumulates and neither can lose an entry. `status` writes nothing at all — a status command that produced `REPORT.md` could never observe `REPORT.md` missing, which is part of the check it exists to perform. It blocks while any of the four artifacts above is absent, as well as on any criterion that is not `PASS` or that is `PASS` without a red record behind it.
