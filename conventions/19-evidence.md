...
A red record says `RED`, `NOT-RED`, `EXEMPT-GUARD` or `NO-BASELINE`. The first three belong to the red phase alone; `NO-BASELINE` is deliberately the same word a criterion's status uses, because 06 §3 gives it one meaning — the check could not run — and a red check that could not run is that case, including a criterion whose command selects no test at all. What a red record never says is `PASS`: the value that satisfies the gate is `RED`, so the two can never be read as the same thing, and `NO-BASELINE` is never treated as evidence of anything.
