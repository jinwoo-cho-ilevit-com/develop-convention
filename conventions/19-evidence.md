...
The red record uses a vocabulary of its own — `RED`, `NOT-RED`, `NO-BASELINE`, `NO-TEST`, `EXEMPT-GUARD` — kept disjoint from the four status words above. A red result that satisfies the gate is `RED`, never `PASS`, so the two can never be read as the same thing. `NO-BASELINE` means the check could not run and is never treated as evidence of anything.
