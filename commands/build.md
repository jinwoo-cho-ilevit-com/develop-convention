...
Claim completion only when `halted` is empty, `unanswered` is zero, `escalations` is empty, every review points row carries an exit, and every merged-whole row's exit is `no blocker` or `skipped (auto)`.
