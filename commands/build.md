...
Then run the workflow at `${CLAUDE_PLUGIN_ROOT}/workflows/build.js` with the Workflow tool, passing `{ planDir, base, lanes, boundaries, conventionsDir, boundariesFrozen: true }` read from the plan as `args`. `base` is the commit the lanes branch from; the reviewers diff against it. `conventionsDir` is the expanded `${CLAUDE_PLUGIN_ROOT}/conventions` — the lanes run in the user's repository, where a bare `conventions/` resolves to nothing. `boundariesFrozen` is your declaration that the step above is done: the workflow refuses to fan out without it, since it is also invokable as a skill and that path skips the freeze.
