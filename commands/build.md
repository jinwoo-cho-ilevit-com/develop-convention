...
5. Run the merged-whole round at the done level's depth (→ `20-review-gate.md` §2 for the lane, `18-work-contract.md` §3 for the depth). Its range is `<base>..<merged head>` pinned as two shas, and each lane's own range is `<base>..<branch>` — resolvable here because branches are deleted only at step 7. The round ends the three ways `20-review-gate.md` §3 sets; write its exit into the review points table.
