...
- Any other test needs a reason the sample run cannot supply: a branch or parser edge the sample does not reach (try adding it as a sample row first), a parallel-lane boundary, or a check 06 or another convention names on its own terms (characterization, a double confirmed against the real system, a standing guard, 08's golden equality test). A test repeating what the sample run asserts is deleted; so is a per-function suite.
...
- A bug fix needs no dedicated regression test: it is decided by reproducing the defect before and after, with that command and output kept as evidence, and the triggering input becomes a sample row where it fits. The fix is checked against the defect's siblings on neighbouring paths before it closes. Assert ML metrics with a tolerance band. CI runs the sample run on CPU to check GPU paths. TODOs/stubs/skips are blockers, not completion.
