...
- The boundary sample carries every value-set member as rows, since the assembled sample run checks only what the sample carries; the merged-whole review compares the merged code against each contract file for what no sample reaches. An object crossing more than one lane boundary gets a single sample that every contract file points at: two samples for one object are two definitions nothing compares.
