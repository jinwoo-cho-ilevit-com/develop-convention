...
- Any other test needs a reason the sample run cannot supply: a branch or parser edge the sample does not reach (try adding it as a sample row first), a parallel-lane boundary, a fixed bug, or a check 06 or another convention names on its own terms (characterization, a double confirmed against the real system, a standing guard, 08's golden equality test). A test repeating what the sample run asserts is deleted; so is a per-function suite.
