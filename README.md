...
- An MCP server's credential is never a value in its config: it is injected into the server's process, not the agent's — a stdio server wrapped as `infisical run --path=<folder> -- <server>`, with `CLAUDE_CODE_MCP_ALLOWLIST_ENV=1` so servers do not inherit the agent's shell environment (its baseline is undocumented, so confirm the server connects with `claude mcp list` rather than forwarding a token). This keeps the secret out of what the agent inherits, not out of its reach — its shell holds the same Infisical login — so in CI the server runs outside the agent's job with its own machine identity, and `INFISICAL_TOKEN` is never forwarded through the server's `env`; a remote server through OAuth or `headersHelper`. A `${VAR}` reference goes only in `env` or headers, never arguments or URL, and its `:-default` is never a secret. The committed config sits inside the secret scan; `claude mcp add --env KEY=value` at local or user scope writes the value to `~/.claude.json`, outside it. The server keeps the upstream credential itself and accepts only tokens issued to it — no token passthrough.

### Agent Sandboxing ([25](conventions/25-agent-sandboxing.md))

- Bound an agent by filesystem and network isolation together; permission prompts and auto mode's classifier are layers inside that boundary, never the boundary. A boundary counts only for what it encloses — Claude Code's OS sandbox covers Bash alone, so it bounds a session whose other tools still pass prompts, and a git worktree covers nothing — and a lane the harness runs unattended is a session under these rules.
- Run a mode that removes every check (`--dangerously-skip-permissions`, `--approval-mode=yolo`; auto mode keeps its classifier and is bounded like a prompted session, plus `permissions.deny` on credential paths and `permissions.ask` on pushes and fetches outside a container) only inside a container or VM that nothing reopens to the host — non-root, egress restricted, no host credential, Docker socket, `--privileged` or host network — and review `.git/hooks` and editor tasks in the bind-mounted workspace before running anything from it on the host. Inject each credential into the process that uses it, not the agent's environment; one that must be out of reach stays outside the boundary, behind a proxy.
- A session holds at most two of untrusted input (issue/PR text, comments, fetched pages, another workflow's artifacts), secrets or private data a callable tool can reach (a private checkout, a logged-in CLI such as a secret manager, `gh` or a cloud CLI), and a tool that changes state or communicates externally; with untrusted input present drop one of the other two, and a session needing all three is either supervised — each write or send passes a person, as in an interactive session; auto mode's classifier does not count — or split, so an unattended lane reads no untrusted text at all: what the work needs from outside is fetched at plan time and reaches the lane as its brief, and a lane that needs more reports the criterion needing it as failed. A subagent's summary of untrusted text is still untrusted — an action on it passes a check or approval first.
- A CI job running an agent gets only the token scopes, persisted credentials and secrets its step uses, declared in the workflow (`permissions: {}` or `contents: read` at the top, `persist-credentials: false` unless it pushes). Every event carrying a stranger's text is untrusted whoever triggered the run — the privileged triggers `pull_request_target`, `workflow_run` (with its artifacts) and `issue_comment`, and the comment and review events a fork's pull request sends to the base repository; on a privileged trigger never check out the untrusted head or interpolate event text into a command.
...
- Completion needs execution evidence, and neither the run nor review is enough alone: a gamed run passes its tests, and review is what catches the shortcut — measured LLM monitors caught 42–89% of cheating that tests had passed, and on public pull requests AI review comments were rarely acted on. A lane judging code runs the code, and reports how many commands it ran; a verdict from a lane that ran none is a reading and says so. Ask the same of the author's evidence — whether any of it ran outside the module under change, since a defect crossing a boundary appears only when something runs both sides.
...
- Each visual in an HTML explainer is designed from the trigger table and the mechanism it must show, never from a form picked first; every figure carries the same accessibility contract — no role on `<figure>` for a static drawing but `role="img"` with a label on the `<svg>` inside, `role="group"` with a label and visible focus once marks are focusable, real controls, and no meaning carried by color alone. Numeric runs use a monospace face with tabular figures while Korean labels keep the body face.
